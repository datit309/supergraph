...
Supergraph là một hệ thống workflow cho AI coding agent, không chỉ là một bộ prompt. Mọi thay đổi không tầm thường đều đi qua:
...
Trên Windows, `hooks/run-hook.cmd` tự tìm Git Bash theo thứ tự: `CLAUDE_CODE_GIT_BASH_PATH`, Git system-wide, Git user-level, rồi `where git.exe`. Vì vậy Git cài bằng winget không cần quyền admin vẫn chạy hook. Nếu không tìm thấy Git Bash, hook in `supergraph: Git Bash not found — hooks skipped` rồi thoát thành công vì hook là non-blocking; skill và MCP vẫn dùng được.
...
`/supergraph:scan` đăng ký hoặc build graph lần đầu và ghi `CBM_PROJECT` vào `.supergraph-env`. Codebase Memory `auto_watch=true` cập nhật index cục bộ bất đồng bộ; các quality gate vẫn kiểm tra index status và reindex rõ ràng trước impact analysis.
...
**Hiện tại: v2.2.4** — Chuyển graph intelligence sang Codebase Memory MCP, thêm kiểm tra lifecycle project/index và sửa hook Windows cho Git Bash cài system-level hoặc user-level.
