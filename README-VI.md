...
## Cài đặt Đồ thị (Graph)

Đồ thị codebase phải được build một lần cho mỗi project. Nó cung cấp năng lượng cho phân tích blast radius, phát hiện hub node, và ranh giới community.

```bash
# Cài tool đồ thị
pip install code-review-graph

# Đăng ký MCP server vào project
code-review-graph install

# Build index đồ thị lần đầu
code-review-graph build
```

Kiểm tra đồ thị hoạt động đúng:

```bash
code-review-graph status
```

Đồ thị tự cập nhật sau mỗi lần ghi file (qua hook `PostToolUse`). Rebuild thủ công sau khi merge lớn:
...
Serena bổ sung code intelligence cấp LSP: tìm tất cả caller, rename an toàn toàn codebase, type diagnostics. Tuỳ chọn nhưng khuyến nghị cho refactor phức tạp.

```bash
# Cài Serena MCP server
pip install serena
...
Sau khi cài, tất cả skill Supergraph tự động dùng Serena tools khi có sẵn (`find_referencing_symbols`, `get_diagnostics_for_file`, `rename_symbol`,...).
...
# 2. Lên kế hoạch trước khi code
/supergraph:plan

# 3. Implement với TDD
/supergraph:tdd

# 4. Auto-fix sau khi code
/supergraph:fix

# 5. Verify trước khi báo xong
/supergraph:verify

# 6. Review trước khi merge
/supergraph:review
```

**Thay đổi nhỏ (1-2 file, <10 dòng)?** Bỏ qua plan → `/supergraph:tdd` → `/supergraph:fix` → `/supergraph:review`
...
| `PostToolUse Write/Edit` | Sau khi ghi file nguồn | Tự động cập nhật index code-review-graph |
...
Copy project scaffolding vào repo của bạn để cả team dùng chung workflow:
...
|---|---|---|
| `.claude-plugin/` | ✅ Có | Plugin manifest + skill dùng chung cả team |
| `.mcp.json` | ✅ Có | Cấu hình MCP server |
| `.code-review-graph/` | ✅ Có | Graph index dùng chung cả team |
| `docs/supergraph/plans/` | ✅ Có | Plan là hợp đồng — cả team cùng theo dõi |
| `CLAUDE.md` | ✅ Có | Hướng dẫn cấp project |
| `.supergraph-env` | ⚠️ Tuỳ | Chứa flag cá nhân như CAVEMAN — thêm vào `.gitignore` nếu dùng riêng |
...
Toàn bộ phân tích đồ thị chạy trên máy bạn. Plan file lưu trong repo. Dịch vụ ngoài duy nhất là Claude Code (Anthropic) cho model inference và tuỳ chọn Serena / code-review-graph, cả hai đều chạy local.
...
**Hiện tại: v2.2.0** — Thêm 8 skill mới (diagnose, handoff, triage, caveman, prd, architecture, prototype, zoom-out), hệ thống từ vựng chung CONTEXT.md, 4 hook automation thông minh.
