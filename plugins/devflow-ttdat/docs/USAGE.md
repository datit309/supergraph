...
Devflow là plugin cho Claude Code, kết hợp hai phương pháp:
...
### Cách 1: Cài từ thư mục local
    ...
    git clone https://github.com/YOUR_USER/devflow.git /tmp/devflow

    # Cài vào project
    cd your-project
    claude plugins install /tmp/devflow

    # Thêm local marketplace
    /plugin marketplace add ./devflow-ttdat

    # Cài plugin
    /plugin install devflow-ttdat@devflow-ttdat

### Cách 2: Copy trực tiếp

    # Copy folder devflow vào .claude/plugins/
    cd your-project
    mkdir -p .claude/plugins
    cp -r /path/to/devflow .claude/plugins/

### Cách 3: Cài từ git (nếu đã push lên GitHub)

    claude plugins install github-user/devflow
...
### Lỗi: Agent không đọc skill
    ...
    ls .claude/plugins/devflow/.claude-plugin/plugin.json
    ls .claude/plugins/devflow/skills/context/SKILL.md

    # Nếu không tồn tại → cài lại:
    claude plugins install ./path/to/devflow
...
### Q: Devflow có bắt buộc dùng TDD không?
...
### Q: Tôi muốn dùng devflow với monorepo được không?
...
### Q: Devflow có chậm không?

A: Không. blast_radius chạy rất nhanh (<2s cho incremental update). Graph đã được index sẵn, chỉ query. So với đọc toàn bộ codebase, devflow nhanh hơn nhiều vì đọc ít file hơn.

### Q: Tôi có thể dùng devflow với Cursor, Copilot không?

A: Devflow được thiết kế cho Claude Code. Các tool khác có thể dùng skills dưới dạng prompt template nhưng không có auto-trigger và MCP integration.
